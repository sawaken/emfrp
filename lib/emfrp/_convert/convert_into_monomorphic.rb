...
module Emfrp
  module Convert
    def convert_into_monomorphic(top, syntax=top, type_tbl={}, func_tbl={})
      puts "enter #{syntax[:name][:desc]}" if syntax.is_a?(Syntax) && syntax.has_key?(:name) && syntax[:name]
      ...
      when FuncCall
        ...
        # patch
        syntax[:func_typing] = Typing::UnionType.new("Func", syntax[:params].map{|x| x[:typing]} + [syntax[:typing]])
