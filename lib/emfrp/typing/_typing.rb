...
module Emfrp
  module Typing
    ...
    def typing(top)
      ...
      top[:datas].sort!{|a, b| a[:order] <=> b[:order]}
    ...
    def typing_tvalues(ftype_tbl, type_defs)
      type_defs.each do |td|
        ...
        td[:tvalues].each do |tv|
          param_types = tv[:params].map{|x| x[:typing] = UnionType.from_type(x[:type], type_var_tbl)}
